...
class datagrid{
	...
	function conectadb($strServer, $strUsername, $strPassword, $strDatabase, $bolUseADOdb=false, $strType='mysqli', $intPort=3306){
		if (!$bolUseADOdb){
			$this->connectionHandler = mysqli_connect("$strServer:$intPort", $strUsername, $strPassword) or
				// $this->SQLerror("mysql_connect('$strServer:$intPort', $strUsername, $strPassword))", mysql_error());
				$this->SQLerror("Failed to connect to Database");
	...
	function desconectar(){
		if ($this->isADO)
			...
			mysqli_close( $this->connectionHandler) or
				$this->SQLerror("mysqli_close( $this->connectionHandler)", mysqli_error($this->connectionHandler));
	}
	...
	function FormatColumn($strfieldName, $strHeader, $fieldWidth=0, $maxlength=0, $inputtype=0, $columnwidth=0, $align= 'center', $Mask='text', $default='', $cutChar=0){
		if ( $strfieldName=="" or !$this->validField( $strfieldName)){
			...
			$pmask = !(strpos(trim($mask),$this->numerics) === false);
				...
				if (strpos($mask,':') >0 ){
					...
					if (strtoupper(substr($mask[1],0,7)) == 'SELECT ') {					#Select data from Table. Format [SELECT key, value FROM table]
						if ($this->isADO){
							...
							$objResult = mysqli_query($this->connectionHandler,$mask[1]) or $this->SQLerror($mask[1], mysqli_error($this->connectionHandler));
							while ($arrResult = mysqli_fetch_array($objResult))
								$maskData[$arrResult[0]]=$arrResult[1];
						}
			...
			# 0=text 1=readonly 2=hidden, 4=non-field relation Image or Calc
	...
	function selectCombo($strCampo, $Actual){
		...
		if (in_array($strCampo,explode(",",str_replace(":select","",$this->search)))){
			...
			if ($this->isADO){
				...
				$objRes = mysqli_query($this->connectionHandler, $strSQL) or $this->SQLerror(mysqli_error($this->connectionHandler), $strSQL);
			}
			$strSelect = "<select size='1' id='dg_schrstr' class='dgSelectpage' >";
			$exitLoop = false; $conteo = 0;
			do{
				if (!$this->isADO){
					if(!$rowRes = mysqli_fetch_array($objRes)) $exitLoop=true;
				}else{
	...
	function grid(){
		...
		switch ($DG_ajaxid) {
					...
					if ($vcode == $md){
						...
						if ($this->isADO){
							if (($objRes = $this->connectionHandler->Execute($strDelete)) === false)
								$this->SQLerror($this->connectionHandler->ErrorMsg(), $strDelete);
						}else{
							$objRes = mysqli_query($this->connectionHandler,$strDelete) or $this->SQLerror(mysqli_error(), $strDelete);
						}
					}else{
					...
					if ($vcode == $md){
						...
						if ($this->isADO){
							if (($objRes = $this->connectionHandler->Execute($strUpdate)) === false)
								$this->SQLerror($this->connectionHandler->ErrorMsg(), $strUpdate);
						}else{
							echo $strUpdate;
							$objRes = mysqli_query($this->connectionHandler, $strUpdate) or $this->SQLerror(mysqli_error(), $strUpdate);
						}
					}else{
						die("<span class='dgError'>".$this->message["errcode"]."</span>");
					}
					...
					if ($isadding){
						...
						if ($this->isADO){
							if (($objRes = $this->connectionHandler->Execute($strSelect)) === false)
								$this->SQLerror($this->connectionHandler->ErrorMsg(), $strSelect);
						}else{
							$objRes = mysqli_query($this->connectionHandler, $strSelect) or $this->SQLerror(mysqli_error($this->connectionHandler), $strSelect);
						}
						if (!$this->isADO){
							$rowRes = mysqli_fetch_array($objRes);
						}else{
			...
			case "6":
				if ($this->addBtn or $this->updBtn){
					...
					if ($this->isADO){
						if (($objRes = $this->connectionHandler->Execute($strSQL)) === false)
							$this->SQLerror($this->connectionHandler->ErrorMsg(), $strSQL);
					}else{
						$objRes = mysqli_query($this->connectionHandler,$strSQL) or $this->SQLerror(mysqli_error(), $strSQL);
					}
				}else{
					die("<div class='dgError'>".$this->message['cannotadd']."</div>");
				}
		...
		if ($this->isADO){
			if (($objSQLpag = $this->connectionHandler->Execute($pagSelect)) === false)
				$this->SQLerror($this->connectionHandler->ErrorMsg(), $pagSelect);
			else
				if (empty($groupby))
			...
			$objSQLpag  = mysqli_query($this->connectionHandler, $pagSelect) or $this->SQLerror(mysqli_error($this->connectionHandler), $pagSelect);
			$arrRow 	= mysqli_fetch_array($objSQLpag);
			if (empty($groupby))
				$intRecords = $arrRow[0];
			else
				$intRecords = mysqli_num_rows($objSQLpag);
		}
